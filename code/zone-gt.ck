...
class Note {
    ...
    Clarinet c => ADSR a => PRCRev n => Gain g => dac;
    0.3 => g.gain;
    0.5 => c.noteOn;
    0.0 => c.startBlowing;
    1.0 => c.vibratoGain;
...
Note held[2];
fun void play(int zone, float z, int prev_zone, float prev_z, int scale[], int side) {
...
while (true) {
    play(calc_rzone(gt.rx, gt.ry), gt.rz, calc_rzone(gt.prev_rx, gt.prev_ry), gt.prev_rz, rscale, 0);
    play(calc_lzone(gt.lx, gt.ly), gt.lz, calc_lzone(gt.prev_lx, gt.prev_ly), gt.prev_lz, lscale, 1);
